...
bool SCOPE(FILE *file){
    ...
    if(token.type != TOKEN_CURLYL_BRACKET)
        return false;
    do{
        // Prvy nacitany charakter musi byt pred funkciou statement aby sme vedeli ci scope nahodou neskoncil mesto statementu
        GET_TOKEN_RAW(token, file);
        if(!STATEMENT(file, token) && token.type != TOKEN_CURLYR_BRACKET)
            return false;
    }while(token.type != TOKEN_CURLYR_BRACKET);
    return true;
}
